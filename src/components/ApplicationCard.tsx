...
interface ApplicationCardProps {
  ...
  onUpdateStatus: (id: string, status: "approved" | "rejected", data: any) => void;
}
...
export default function ApplicationCard({ application, onUpdateStatus }: ApplicationCardProps) {
  ...
  const [credentials, setCredentials] = useState({
    username: application.email.split('@')[0].toLowerCase().replace(/[^a-z0-9]/g, ''),
      ...
      <div className="px-6 py-4 grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm">
              <span className="font-medium">Purpose:</span> {purposeMap[application.purpose] || application.purpose}
            </p>
          </div>
        </div>
      ...
      {application.status === "approved" && application.credentials && (
        <div className="px-6 py-4 bg-green-50 border-t border-green-200">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <p className="text-sm"><span className="font-medium">Username:</span> {application.credentials.username}</p>
            </div>
            <div>
              <p className="text-sm"><span className="font-medium">Password:</span> {application.credentials.password}</p>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl p-6 max-w-md w-full">
            ...
            <p className="text-sm text-gray-600 mb-4">
              Please review or modify the generated credentials for {application.firstName} {application.lastName}.
              These credentials will be emailed to {application.email}.
            </p>
            ...
            <div className="space-y-4 mb-6">
              ...
              <div>
                ...
                <div className="flex space-x-2">
                  ...
                  />
                  <button
                    onClick={() => setCredentials({...credentials, password: generateRandomPassword(10)})}
                    type="button"
      ...
      {showRejectDialog && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl p-6 max-w-md w-full">
            ...
            <p className="text-sm text-gray-600 mb-4">
              Please provide a reason for rejection. This message will be sent to {application.email}.
            </p>

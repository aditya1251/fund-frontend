...
export default function LoanTemplateBuilder() {
  ...
  const { data: templateData, refetch } =
    useGetLoanTemplateByIdQuery(FIXED_TEMPLATE_ID);
  ...
  const addFieldToPage = (i: number) => {
    ...
    const updatedPages = template.pages.map((p, idx) =>
  ...
  const removePage = (i: number) => {
    const newPages = template.pages.filter((_, idx) => idx !== i);
    newPages.forEach((p, idx) => (p.pageNumber = idx + 1));
    setTemplate({ ...template, pages: newPages });
  ...
  return (
    <div className="max-w-5xl mx-auto py-10 space-y-6">
      <h2 className="text-2xl font-bold">Edit Quick Loan Template</h2>
      ...
      <div className="border-t pt-6 space-y-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          />
          <Button onClick={addPage} className="bg-black text-white">
      ...
      <div className="flex gap-4 border-t pt-6">
        <Button onClick={saveTemplate} className="bg-[#ffd439] text-black">
          <Save className="w-4 h-4" /> Save Quick Loan
        </Button>
      </div>
    </div>
  );

...
const mockLoanData = [
  {
    ...
    rejectionMessage: "",
    status: "approved",
    values: [{ fields: [{ value: "Alice Johnson" }, { value: "alice.j@example.com" }, { value: "555-0101" }] }],
  },
  {
    _id: "LN002",
    loanType: "insurance",
    ...
    status: "rejected",
    values: [{ fields: [{ value: "Bob Williams" }, { value: "bob.w@example.com" }, { value: "555-0102" }] }],
  },
  {
    _id: "LN003",
    loanType: "insurance",
    ...
    rejectionMessage: "",
    status: "pending",
    values: [{ fields: [{ value: "Charlie Brown" }, { value: "charlie.b@example.com" }, { value: "555-0103" }] }],
  },
];

const mockTemplateData = [
    { id: 'template1', name: 'Personal Accident', description: 'Cover for accidents and more words to test wrapping', icon: 'user' },
    { id: 'template2', name: 'Home Shield', description: 'Protection for your house and property from various perils', icon: 'home' },
    { id: 'template3', name: 'Auto Secure', description: 'Comprehensive coverage for your vehicle, including theft and damage', icon: 'car' },
    { id: 'template4', name: 'Business Policy', description: 'Insurance for your business operations and assets', icon: 'building' },
];

const useGetLoansByDsaIdQuery = (dsaId: string) => ({ data: mockLoanData, isLoading: false });
const useGetLoanTemplatesByTypeQuery = (type: string) => ({ data: mockTemplateData, isLoading: false });

const RequireFeature = ({ children, feature }: { children: React.ReactNode; feature?: string }) => <>{children}</>;
...
const Input = (props: React.InputHTMLAttributes<HTMLInputElement>) => <input {...props} className={`border bg-white ${props.className || ''}`} />;
const Select = ({ children, ...props }: React.SelectHTMLAttributes<HTMLSelectElement>) => <select {...props} className={`border bg-white ${props.className || ''}`}>{children}</select>;
...
const Tabs = ({ children }: { children: React.ReactNode }) => {
  ...
  const contextValue = useMemo(() => ({ value: activeValue, setValue: setActiveValue }), [activeValue]);
  return (
    <React.Fragment> {/* Use fragment to avoid introducing an extra div if not strictly needed */}
...
const TabsList = ({ children, className }: { children: React.ReactNode; className?: string }) => (
  ...
  <div className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 bg-white p-6 rounded-lg shadow-sm ${className}`}>
    {children}
  </div>
);

const TabsTrigger = ({ children, isActive, ...props }: { children: React.ReactNode; isActive: boolean; onClick?: () => void }) => {
    const baseClasses = "w-full flex items-center gap-4 text-left rounded-md border text-sm transition-all hover:shadow-md";
    ...
    const activeClasses = "bg-[#f5d949] border-yellow-300 font-semibold text-black"; // Added text-black
    const inactiveClasses = "bg-white border-neutral-600 text-gray-800"; // Adjusted text color
    
    return (
        <button {...props} className={`${baseClasses} ${responsivePadding} ${consistentMinHeight} ${isActive ? activeClasses : inactiveClasses}`}>
            {children}
        </button>
    );
};

const TabsIcon = ({ children, isActive }: { children: React.ReactNode; isActive?: boolean }) => (
  // Ensured text-black is default, and active state changes text color
  <span className={`w-5 h-5 ${isActive ? 'text-black' : 'text-gray-600'}`}>{children}</span>
);

const TabsLabel = ({ children }: { children: React.ReactNode }) => (
  <div className="flex flex-col flex-grow"> {/* Use flex-col to stack label and description */}
...
const TableWrapper = ({ children, className }: { children: React.ReactNode; className?: string }) => <div className={`border rounded-lg ${className}`}>{children}</div>;
const TableHeadings = ({ columns }: { columns: string[] }) => (
  <thead>
    <tr className="bg-gray-50">
      {columns.map((col, i) => (
        <th key={i} className="p-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{col}</th>
      ))}
    </tr>
  </thead>
);
const TableRow = ({ row }: { row: React.ReactNode[] }) => (
  <tr className="border-b hover:bg-gray-50">
    {row.map((cell, i) => (
      <td key={i} className="p-3 text-sm text-gray-700 whitespace-nowrap">{cell}</td>
    ))}
  </tr>
);
const EmailCell = ({ email }: { email: string }) => <a href={`mailto:${email}`} className="text-blue-600 hover:underline">{email}</a>;
    ...
    <RequireFeature feature="Insurance">
      ...
      <div className="max-w-full overflow-hidden px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
          <h4 className="font-semibold text-black mb-4 sm:mb-0">Insurance Types</h4>
              ...
              return (
                ...
                >
                  ...
                  <TabsTrigger isActive={isActive}>
                    ...
                    <TabsLabel>
                      {/* Name styling */}
                      <div className={`text-sm ${isActive ? 'text-black' : 'text-gray-800'}`}>{template.name}</div>
        ...
        <div className="mt-6">
          <div className="py-4">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
              <h4 className="text-lg font-semibold text-black mb-4 md:mb-0">Insurance Leads</h4>

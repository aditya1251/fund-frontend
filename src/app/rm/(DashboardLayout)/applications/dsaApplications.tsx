...
export default function DsaApplicationsPage({
  ...
  const [commissionLoan, setCommissionLoan] = useState<any | null>(null);
                ...
                >
                  ...
                  <div className="flex gap-2 mt-4">
                    ...
                    <CommissionButton
                      loan={loan}
                      onAdd={(refetch) => {
                        setCommissionLoan(loan);
                        // store refetch for later
      ...
      {commissionLoan && (
        <div className="fixed inset-0 z-50 flex justify-center items-center bg-black/40 backdrop-blur-sm">
          <div className="bg-white w-full max-w-md p-6 rounded-xl border-2 border-black shadow-[6px_6px_0_0_#000] relative">
            ...
            <h3 className="text-xl font-bold mb-4">Set Commission</h3>
            ...
            <div className="flex justify-end gap-2">
              ...
              <button
                onClick={async () => {
                  if (!commissionAmount) return alert("Enter amount");
                  try {
                    await createCommission({
                      data: {
                        loanId: commissionLoan._id,
                        amount: Number(commissionAmount),
                        status: "credited",
                      },
                    }).unwrap();
                    commissionLoan.commissionRefetch?.();
                    setCommissionLoan(null);
                    setCommissionAmount("");
                  } catch (err) {
...
const CommissionButton = ({
  ...
  loan: any;
  onAdd: (refetch: () => void) => void;
}) => {
  const {
    data: commission,
    isLoading,
    refetch,
  } = useGetCommissionByLoanQuery(loan._id);
